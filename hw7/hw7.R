...
speechYr <- sapply(strsplit(tempDates,", ",fixed = TRUE),"[[",2)
speechMo <- sapply(strsplit(tempDates," [[:digit:]]",fixed = FALSE),"[[",1)
...
speechesL<-list()
for(i in 1:221){
  str = paste(speeches[(breaks[[i]]+6):(breaks[[i+1]]-2)], sep=" ",collapse=" ")
  speechesL[[i]]=unlist(strsplit(str,split="\\.|\\!|\\?"))
}
last.speech<-paste(speeches[(breaks[[222]]+6):(length(speeches)-2)], sep=" ",collapse=" ")
last.speech<-unlist(strsplit(last.speech,split="\\.|!\\|\\?"))
speechesL[[222]]<-last.speech

speechesL2<-list()
for(i in 1:221){
  str = paste(speeches[(breaks[[i]]+6):(breaks[[i+1]]-2)], sep=" ",collapse=" ")
  speechesL2[[i]]=strsplit(str,split="\\.|\\!|\\?")
}
last.speech<-paste(speeches[(breaks[[222]]+6):(length(speeches)-2)], sep=" ",collapse=" ")
last.speech<-strsplit(last.speech,split="\\.|!\\|\\?")
speechesL2[[222]]<-last.speech


#unlist speeeches, take first speech and put into the first element of the empty list
#vector("list", n.speeches), every speech into its own element in the vetor(list)
#then use strsplit to split upeach element by \\.|!\\|\\?"
...
test.sentences<-c("Mr Jackson \n 19 likes to eat! Applause. recieve recieve, laughter I'm")
...
vectorcounts<- function(eachspeech){
  counts<-table(eachspeech)
  counts2<-match(names(counts),uniqueWords)
  emptyVec = rep(0, length(uniqueWords))
  names(emptyVec) = uniqueWords
  emptyVec[counts2]<-as.vector(counts)
  return(emptyVec)
}
...
wordVecs <- lapply(speechWords,vectorcounts)
...
chars<-unlist(lapply(speechesL2,nchar))
...
no.uniquePresidents<-length(unique.Presidents)
presidentWordMat <- matrix(,nrow=no.uniqueWords,ncol=no.uniquePresidents)
  for(i in 1:no.uniquePresidents){
    no.speeches<-which(presidents==unique.Presidents[i])
    if(length(no.speeches)>1){
    presidentWordMat[,i]<-rowSums(wordMat[,no.speeches])
  }
  if(length(no.speeches)==1){
    presidentWordMat[,i]<- wordMat[,no.speeches]
  }
}
colnames(presidentWordMat)<-unique.Presidents
...
speechesDF3<-speechesDF[,1:3]
presParty <- unlist(unique(speechesDF3)$party)
...
plot(mds, xlab="",ylab="", main="Presidents",type='n')
text(mds,labels=unique.Presidents,col = cols[presParty[1:41]])
...
plot(speechYr,sentences,xlab="Year",ylab="Sentences",main="President's Senteces per Year")
plot(speechYr,words,xlab="Year",ylab="Words",main="President's Words per Year")
plot(speechYr,chars,xlab="Year",ylab="Characters",main="President's Characters per Year")
plot(speechYr,(chars/words),xlab="Year",ylab="Average Word Length",main="President's Average Word Length per Year")
plot(speechYr,(words/sentences),xlab="Year",ylab="Average Sentence Length",main="President's Average Sentence Length per Year")
