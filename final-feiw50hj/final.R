...
github.name <- "jabdelmesieh"
email.address <- "jabdelmesieh@berkeley.edu"
...
# y <- <your code here>
y<-list((1:1000),2*(1:1000),3*(1:1000),4*(1:1000),5*(1:1000),6*(1:1000),7*(1:1000),8*(1:1000),9*(1:1000),10*(1:1000))
...
set.seed(42)
#z <- <your code here>
z<-sample(c(T,F),5000,replace=TRUE,prob=c(0.3,0.7))
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
#v <- <your code here>
v<-sample(c(1,2,3,4,5,6),100,replace=TRUE)
...
# 
# t2 <- <your code here>
t22<-infants[infants$ed=="College",]
t2<- table(t22$smoke, t22$number)
...
line<-lm(mtcars$qsec~mtcars$hp)
abline(a=20.55635,b=-0.01846)
...
plot(mtcars$disp~mtcars$wt, xlab="Car Weight",ylab="Car Displacement",col=as.numeric(mtcars$am+2))
...
plot(WorldBank$life.expectancy~WorldBank$fertility,pch=".", col=as.numeric(WorldBank$region),xlab="Country Fertility Rate", 
     ylab="Country Life Expectancy", main='Fetility Rate vs Life Expectancy')
abline(v=2.1)
# [4 pts]
...
par(mfrow=c(1,2))
yr1960<-WorldBank[WorldBank$year==1960,]
yr2013<-WorldBank[WorldBank$year==2013,]
plot(yr1960$life.expectancy,yr1960$fertility,pch=17, col=as.numeric(WorldBank$region),xlab="Country Fertility Rate", 
     ylab="Country Life Expectancy", main='Fetility Rate vs Life Expectancy in 1960')
plot(yr2013$life.expectancy,yr2013$fertility,pch=17, col=as.numeric(WorldBank$region),xlab="Country Fertility Rate", 
     ylab="Country Life Expectancy", main='Fetility Rate vs Life Expectancy in 2013')
...
par(mfrow=c(1,1))
hist(WorldBank[WorldBank$lending=="IDA",]$GDP, xlab="GDP", main="GDP for Countries with IDA Lending",breaks=250)
...
max.diff.rain<- sapply(rain, diff)
max.diff.rain<- sapply(max.diff.rain,max)
...
raining<-function(m){
  for(i in 1:length(m)){
  length(m[[i]]!=0)/length(m[[i]])
}}
prop.rain<-sapply(rain,raining)
######
# [3 pts]
...
# use an apply statment to do the the plotting
par(mfrow=c(2,3))
lapply(rain,hist)
...
#function <- GenNorm(){
  ...
  #
#}
GenNorm<-function(n=1000,mean=0,sd=1,plot.hist=TRUE){
  normal<-rnorm(n,mean,sd)
...
PermDiff <- function(cases, controls, k=5000){
  cases1<-sample(c(cases,controls),k, replace=FALSE)
  controls1<-sample(c(cases,controls,-c(cases1)),length(controls), replace=FALSE)
...
NumJackpot<-function(k, B){
  for(i in 1:B){
  counter<-
  winning[i]-sample(seq(1,19,by=1),3)
  a[i,]<-replicate(k,sample(seq(1,19,by=1),3, replace=FALSE))
  counter[i]<-counter+sum(all(a[[i,]]==winning[i]))
  return(counter)
  }
}
########
